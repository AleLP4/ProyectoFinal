...
//Llamar al modelo desde un metodo
    ...
    public void conectarVistas(PanelControl pc, PanelTablero pt) {
        ...
        ubicarEnHome();
        panelControl.setTxtNumeroDeDado(0);
        panelControl.setTxtTurnoDeFicha(colores.idAtexto(colorJugadorId));
    ...
    private void cambiarTurnoYActualizar() {
        ...
        String nombre;
        if (turno) {
            nombre = colores.idAtexto(colorJugadorId);
        } else {
            nombre = colores.idAtexto(colorOponenteId);
        }
        panelControl.setTxtTurnoDeFicha(nombre);
    }

    // Metodo jugar, tiene la logica del juego
    public void jugar() {
        int textoDado = dado.tirada();//usamos el metodo lanzar
        panelControl.setTxtNumeroDeDado(textoDado);
    ...
    private void EnRuta(Ficha ficha, int idColor, int tiro) {
        ...
        if (reglas.posicionEntrada(idColor, desde, tiro)) {
...
//fase3
    ...
    private void PreguntaConReglas(Ficha f, int idColor, int dificultad, boolean mandarHomeSiFalla) {
        boolean escucha = preguntas.preguntasV(dificultad);
